...
def kg_embedding(inputs,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        f_transe = open('movielens_attributes/books_kg.txt', 'r', encoding='UTF-8')
        id2embedding = []
...
def multihead_attention_kg(queries,
                        keys,
                        num_units=None,
                        num_heads=8,
                        dropout_rate=0,
...
def caser_layer(msl,
    ...
    for kernal in range(1, msl + 1):
        hconv = tf.layers.conv1d(usr_seq,  h_size, kernal)
    ...
    vconv = tf.layers.conv1d(u, v_size, 1)
    ...
    s = tf.nn.dropout(s, keep_prob=keep_prob)
    z = tf.layers.dense(s, units=emb_size, use_bias=True)
